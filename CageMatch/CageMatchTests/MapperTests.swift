...
class MapperTests: XCTestCase {
    func testNavigationResponse() -> Void {
        ...
        guard let response = NavigationResponseMapper.from(json) else {
            XCTFail("Error decoding item")
            return
        }
        
        let mainChildren = response.mainNavigationItems
        
        XCTAssert(response.mainNavigationItems.count == 5, "Warning - should have 5 main children. Got \(response.mainNavigationItems.count)")
    ...
    func testBrokenMainNavigationItems() {
        ...
        print("BROKEN MAIN MAV \(NavigationResponseMapper.from(json)?.mainNavigationItems)")
    
        XCTAssertNil(NavigationResponseMapper.from(json), "Should fail")
    }
    ...
    func testBrokenChildNavigationItems() {
        ...
        guard let response = NavigationResponseMapper.from(json) else {
            XCTFail("Should have a response")
            return
        }
        
        XCTAssert(response.mainNavigationItems.count == 1, "Warning - should have 1 main children because help & info was corrupt, got \(response.mainNavigationItems.count)")
    }
}
